...
- (void)setUserDefaultDic:(NSString *)key withDic:(NSDictionary *)dic {
    NSString *newKey = @"";
...
- (NSMutableArray *)getUserDefalutMutableArray:(NSString *)key{
    
    
    NSData *data = [[NSUserDefaults standardUserDefaults] objectForKey:key];
    NSMutableArray *savedArray = [NSKeyedUnarchiver unarchiveObjectWithData:data];
    
    return savedArray;
}

- (void)setUserDefaultMutableArray:(NSString *) key withMutableArray:(NSMutableArray *)array{
...
- (void)removeUserDefaultMutableArray:(NSString *)key{
...
- (void)fnHideModalTransparent :(UIViewController *)vcMain {
    
    CATransition* transition = [CATransition animation];
    transition.duration = 0.3;
    transition.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
    transition.type = kCATransitionFade;
    [vcMain.navigationController.view.layer addAnimation:transition forKey:kCATransition];
    
    [vcMain dismissViewControllerAnimated:YES completion:nil];
}
